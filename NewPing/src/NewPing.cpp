...
NewPing::NewPing(uint8_t trigger_pin, uint8_t echo_pin, unsigned int max_cm_distance, bool newMux, int S0, int S1, int S2, int S3) {
	muxS0 = S0;
	muxS1 = S1;
	muxS2 = S2;
	muxS3 = S3;
	mux = newMux;
#if DO_BITWISE == true
	if(mux) seleccionarPuerta(TrigPin);
	_triggerBit = digitalPinToBitMask(trigger_pin); // Get the port register bitmask for the trigger pin.
	if(mux) seleccionarPuerta(EchoPin);
