...
QTRSensorsAnalog::QTRSensorsAnalog(unsigned char* pins,
  unsigned char numSensors, int S0, int S1, int S2, int S3, char newType,
...
void QTRSensorsAnalog::readPrivate(unsigned int *sensor_values)
{
    ...
    for (j = 0; j < _numSamplesPerSensor; j++)
    {
        for (i = 0; i < _numSensors; i++)
        {
			if(type == 'L') seleccionarPuerta(i + 1);
			else if(type == 'C') seleccionarPuerta(i + 3);
			else if(type == 'R') seleccionarPuerta(i + 11);
